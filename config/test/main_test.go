...
func TestBareConfig(t *testing.T) {
	...
	for key, value := range strFlags {
		if value.flag == nil {
			t.Errorf("flag: expected %s to be non-nil but got %v",
				key, value.flag)
		}
	}
	...
	for key, value := range boolFlags {
		if value.flag != nil {
			t.Errorf("flag: expected %s to be nil but got %T",
				key, *value.flag)
		}
		if value.cfg != nil {
			t.Errorf("cfg: expected %s to be nil but got %T",
				key, *value.cfg)
		}
	}
	for key, value := range webBasicAuthFlags {
		if value.flag != nil {
			t.Errorf("flag: expected %s to be nil but got %v",
				key, value.flag)
		}
		if value.cfg != nil {
			t.Errorf("cfg: expected %s to be nil but got %v",
				key, value.cfg)
