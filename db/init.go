...
// chmodFile changes a file's permissions (overridable for tests).
// see [os.Chmod].
...
func (api *api) initialise() (ok bool) {
	...
	// Holds password/session/token hashes when auth is enabled, so drop any
	// group/other access.
	if info, err := os.Stat(databaseFile); err == nil {
		if perm := info.Mode().Perm(); perm&0o077 != 0 {
			tightened := perm &^ 0o077
			if err := chmodFile(databaseFile, tightened); err != nil {
				logx.Error(err, logFrom, true)
			} else {
				logx.Info(
					fmt.Sprintf(
						"Tightened %q permissions from %#o to %#o",
						databaseFile, perm, tightened,
					),
					logFrom, true)
			}
		}
