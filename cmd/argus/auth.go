...
// pruneTicker supplies the prune loop's tick channel and its stop function
// (overridable in tests).
var pruneTicker = func() (<-chan time.Time, func()) {
...
func setupAuth(
	...
	g.Go(func() error {
		tick, stopTicking := pruneTicker()
		defer stopTicking()
		for {
			if ctx.Err() != nil {
				return nil
			}
			if err := sessions.PruneExpired(ctx); err != nil && ctx.Err() == nil {
				logx.Error(err, logFrom, true)
			}
			if err := authStore.DeleteExpiredAPITokens(ctx); err != nil && ctx.Err() == nil {
				logx.Error(err, logFrom, true)
			}
			select {
			case <-ctx.Done():
