...
// and registering it with the DB.
//
// Response:
//
//	201 Created: JSON of the created group.
//	400 Bad Request: on a missing name, malformed body, or invalid grant.
...
func (api *API) httpGroupCreate(w http.ResponseWriter, r *http.Request) {
	...
	name, err := normaliseName("name", request.Name)
	if err != nil {
		failRequest(&w, err, http.StatusBadRequest)
		return
	}
	if err := validateFieldLength("description", request.Description); err != nil {
		failRequest(&w, err, http.StatusBadRequest)
		return
	}
	...
	group, err := api.auth.Store.CreateGroup(
		r.Context(),
		name, request.Description, request.Permissions,
	)
	if err != nil {
...
func (api *API) kickGroupMemberWebSocketClients(
	r *http.Request,
	groupID string,
	...
	defer cancel()
	memberIDs, err := api.auth.Store.UserIDsInGroup(ctx, groupID)
	if err != nil {
		logx.Error(err, logFrom, true)
		return
	}
