...
// Login rate limiting: fixed windows keyed on (client IP, username), so a
// shared address cannot lock out accounts other than the one being guessed.
const (
...
// check reports whether key is within the [loginLimitAttempts] limit, without recording an attempt
// (and prunes expired windows).
func (l *loginLimiter) check(key string) bool {
	now := timeNow()
