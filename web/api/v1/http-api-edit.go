...
func (api *API) mayUpdateService(r *http.Request, serviceID string, tags []string) bool {
	...
	target := rbac.Target{ServiceID: serviceID, Tags: tags}
	return authCtx != nil &&
...
func (api *API) httpServiceEdit(w http.ResponseWriter, r *http.Request, reqType serviceAction) {
	...
	if api.Config.ServiceIDTaken(serviceID, newService) {
		failRequest(
			&w,
			fmt.Errorf(
				"%s %q failed, a service with this id or name already exists",
				reqType, newService.ID,
			),
			http.StatusBadRequest,
		)
		return
...
func serviceEditAffectsVisibility(
...
//	400 Bad Request: Error message.
//	404 Not Found: Unknown service.
//	500 Internal Server Error: Failed to prune the service's permission grants.
...
func (api *API) httpNotifyTest(w http.ResponseWriter, r *http.Request) {
	...
	if parsedPayload.ServiceIDPrevious != "" {
		...
		if svc != nil {
			// Require service:update to inherit secrets.
			if !api.mayUpdateService(r, parsedPayload.ServiceIDPrevious, svc.Dashboard.Tags) {
				failRequest(&w, errForbidden, http.StatusForbidden)
				return
			}
