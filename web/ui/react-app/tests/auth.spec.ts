...
import type { PermissionCatalogue } from '@/types/auth';
...
test.describe('Authentication', () => {
	...
	/** Dismisses the open tooltip, so the next hover has the page to itself. */
	...
	test('builds a service-scoped grant through the grant editor', async () => {
		...
		// The target input mounts on the scope change; it is required for a
		// non-global scope.
		await page.getByLabel('Grant 0 target').fill(SERVICE);
		await page.getByRole('button', { name: 'Save' }).click();
	...
	test('describes every selectable permission, in the header tooltip and on the row', async () => {
		// Every permission a grant row can select comes from the API catalogue.
		const response = await adminAPI.get('api/v1/permissions');
		expect(response.status()).toBe(200);
		const { resources } = (await response.json()) as PermissionCatalogue;
		const selectable = resources.flatMap((resource) =>
			resource.actions.map(({ action }) => `${resource.name}:${action}`),
		);
		...
		// The header tooltip is a description list: permission | description.
		await page.getByLabel('What each permission allows').hover();
		const tooltip = page.getByRole('tooltip');
		await expect(tooltip).toBeVisible();
		const terms = await tooltip.locator('dt').allTextContents();
		const descriptions = await tooltip.locator('dd').allTextContents();
		...
		// It covers the catalogue exactly, and describes every entry.
		expect(terms).toHaveLength(selectable.length);
		for (const permission of selectable) {
			const description = described.get(permission);
			expect(
				description,
				`${permission} is absent from the header tooltip`,
			).toBeDefined();
			for (const empty of ['', 'No description available.']) {
				expect(description, `${permission} has no description`).not.toBe(empty);
			}
		}

		/** The header tooltip's description of permission. */
		const describedAs = (permission: string): string => {
			const description = described.get(permission);
			if (!description) {
				throw new Error(`${permission} is absent from the header tooltip`);
			}
			return description;
		};

		// AND: each permission's own row repeats that same description.
		await page.getByRole('button', { name: 'Add permission' }).click();
		for (const resource of resources) {
			await page.getByLabel('Grant 0 resource').click();
			await page
				.getByRole('option', { exact: true, name: resource.name })
				.click();
			for (const { action } of resource.actions) {
				await page.getByLabel('Grant 0 action').click();
				await page.getByRole('option', { exact: true, name: action }).click();

				const permission = `${resource.name}:${action}`;
				await page.getByLabel(`What ${permission} allows`).hover();
				const rowTooltip = page.getByRole('tooltip');
				await expect(rowTooltip.locator('dt')).toHaveText(permission);
				await expect(rowTooltip.locator('dd')).toHaveText(
