...
/**
 * Converts the notifier.(params|url_fields) for the API.
 *
 * @param obj - The object to convert.
 * @param notifyType - The type of the notifier.
 * @returns The object converted to string values.
 */
	...
	Object.entries(obj).reduce((result, [key, value]) => {
		if (typeof value === 'object') {
			...
			if ('responders' === key || 'visibleto' === key) {
				// `value` empty means defaults used. Skip.
				if (
				...
				// Ntfy - `label` empty means defaults in use. Skip.
				// OpsGenie - `arg` empty means defaults in use. Skip.
				if (
				...
				// `value` empty means defaults in use. Skip.
				if (
					(value as NotifyOpsGenieTarget[]).find((item) =>
						isEmptyOrNull(item.value),
					)
...
/**
 * Flattens the string map for the API.
 *
...
/**
 * Converts the NTFY actions for the API.
 *
...
/**
 * Converts the OpsGenie targets for the API.
 *
