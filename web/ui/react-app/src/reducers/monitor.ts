...
export default function reducerMonitor(
	...
	switch (action.type) {
			...
			switch (action.sub_type) {
				case 'INIT': {
					if (action.service_data) {
						...
						state.tagsLoaded = Object.values(state.service).every(
							(s) => s.loading === false,
						);
					}
					break;
				...
				case 'ORDER': {
					...
					state = {
						...
						service: action.order.reduce(
							(acc: Record<string, ServiceSummary>, id: string) => {
		...
		case 'VERSION': {
			...
			if (!(id in state.service)) return state;
		...
		case 'EDIT': {
			...
			let oldService: ServiceSummary | undefined;
			let editedService: ServiceSummary;
			...
			} else if (newServiceData.id in state.service) {
		...
		case 'DELETE': {
			...
			if (action.sub_type in state.service)
				state = removeService(state, state.service[action.sub_type].id);
...
const addService = (
	...
	old_service_data?: ServiceSummary,
	add_to_order = true,
) => {
	...
	if (service_data.name !== old_service_data?.name) {
		...
		if (old_service_data?.name) newNames.delete(old_service_data.name);
		if (service_data.name) newNames.add(service_data.name);
	}
 ...
 * @param state - The current state of the monitor.
 * @param tags_of_replacement_service - The tags of the replacement service.
...
const removeService = (
	...
	if (!(id in state.service)) return state;
	const service = state.service[id];

	// Remove the name from the names Set.
	let newNames: Set<string> | undefined;
	if (service.name) {
		newNames = new Set(state.names);
		newNames.delete(service.name);
	}

	// Remove the service from the service object.
	const { [id]: _, ...newService } = state.service;

	// Remove any now unused tags.
	state = updateTags(state, service.tags, tags_of_replacement_service, id);

	// Remove this service from the order array if not being replaced.
	const newOrder =
...
const updateTags = (
	...
	for (const service of Object.values(state.service)) {
		...
		for (const tag of service.tags ?? []) usedTags.add(tag);
	}

	// Add tags new tags.
	let tagsAdded = false;
	for (const tag of newServiceTags) {
		// New tag.
	...
	// If tags added/removed update the state.
