...
/** The API's expires_in for a form's expiry choice ('never' omits it). */
const expiresInOf = (values: TokenFormValues): string | undefined => {
	if (values.expiry === 'never') return undefined;
	return values.expiry === 'custom'
				...
				>
					<Field className="gap-2" data-invalid={!!errors.name}>
						...
						<Input
							...
							{...form.register('name')}
						/>
						<FieldError errors={[errors.name]} id="token-name-error" />
					</Field>
							...
							<Controller
								...
								render={({ field }) => (
									<Select onValueChange={field.onChange} value={field.value}>
										<SelectTrigger className="w-full" id="token-expiry">
											<SelectValue />
										</SelectTrigger>
										<SelectContent>
					...
					<DialogFooter>
						<Button
							...
							type="submit"
						>
							<TextOrLoading loading={isPending} text="Create" />
						</Button>
					</DialogFooter>
				</form>
			</DialogContent>
