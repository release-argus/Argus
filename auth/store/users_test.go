		...
		},
		{
			...
			setup: func(t *testing.T, store *Store) {
				if _, err := store.db.Exec(`
					...
					VALUES ('x', 'x', 'x', 'not-a-time', 'not-a-time');`,
				); err != nil {
					t.Fatalf(
						"%s setup failed: %v",
						packageName, err,
...
func TestStore_CreateUser__requiresPassword(t *testing.T) {
	// GIVEN: a Store. Every account authenticates locally, so there is no
	// valid way to create one without a password.
	store := testStore(t)

	// WHEN: a user is created with no password hash.
	_, err := store.CreateUser(
		t.Context(), "nopass", "", "", "", nil,
	)

	// THEN: it is rejected.
	if got, want := err, ErrNoPassword; !errors.Is(got, want) {
		t.Errorf(
			"%s\nCreateUser() with an empty hash\ngot:  %v\nwant: %v",
...
func TestStore_CreateUser__duplicateGroupNames(t *testing.T) {
	...
	if got, want := len(grants), seedGrantCount(t, GroupViewer); err != nil || got != want {
		t.Errorf(
			"%s grant count mismatch\ngot:  %d, err=%v\nwant: %d",
			prefix, len(grants), err, want,
		)
	}
}
...
func TestStore_UpdateUser(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "empty password hash rejected",
			patch:   UserPatch{PasswordHash: new("")},
...
func TestStore_UpdatePasswordHash(t *testing.T) {
	...
	// GIVEN: Stores holding a user whose password hash is [oldHash].
	tests := []struct {
		name       string
		hash       string
		dropUsers  bool // Unreadable table first.
		wantErr    error
		errRegex   string
		wantStored string // Hash held afterwards.
	}{
		{
			name:       "valid/replaces the stored hash",
			hash:       newHash,
			wantStored: newHash,
		},
		{
			name:       "invalid/empty hash keeps the old one",
			hash:       "",
			wantErr:    ErrNoPassword,
			wantStored: oldHash,
		},
		{
			name:      "invalid/users table unreadable",
			hash:      newHash,
			dropUsers: true,
			errRegex:  `^update password hash`,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			store := testStore(t)
			user := mustCreateUser(t, store, username, oldHash)
			if tc.dropUsers {
				dropTable(t, store, "users")
			}

			prefix := fmt.Sprintf(
				"%s\nUpdatePasswordHash(%q)",
				packageName, tc.hash,
			)

			// WHEN: UpdatePasswordHash is called.
			err := store.UpdatePasswordHash(t.Context(), user.ID, tc.hash)
			...
			// AND: the stored hash is left as expected.
			if tc.dropUsers {
				return
			}
			creds, err := store.LocalCredentials(t.Context(), username)
			if err != nil || creds.PasswordHash != tc.wantStored {
				t.Errorf(
					"%s stored hash mismatch\ngot:  %q, err=%v\nwant: %q",
					prefix, creds.PasswordHash, err, tc.wantStored,
				)
			}
		})
	}
...
func TestStore_CreateFirstAdmin(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "another setup completed between the count and the insert",
			raceUser: true,
			wantErr:  ErrSetupComplete,
		},
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.raceUser {
				...
				hashPassword = func(plaintext string) (string, error) {
					mustCreateUser(t, store, "racer", "")
					return hashPasswordHad(plaintext)
				}
				t.Cleanup(func() { hashPassword = hashPasswordHad })
			}
