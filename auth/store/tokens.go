...
// maxAPITokensPerUser caps the number of tokens a single user may hold.
...
func (s *Store) CreateAPIToken(
	...
	result, err := s.db.ExecContext(ctx, `
		...
		token.Prefix,
		token.CreatedAt.Format(timeFormat),
		expires,
		token.UserID,
		maxAPITokensPerUser,
	)
	...
	if err != nil {
		return "", nil, fmt.Errorf("check API token insert: %w", err)
	}
	if rows == 0 {
...
func scanAPIToken(row scanner) (*APIToken, error) {
	...
	if err := row.Scan(
		...
		timeText{&token.CreatedAt}, nullTimeText{&token.ExpiresAt}, nullTimeText{&token.LastUsedAt},
	); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
