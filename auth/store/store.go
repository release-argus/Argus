...
// syncPermissions reconciles the permissions table with the current catalogue.
//
// Missing permissions are inserted. Permissions absent from the catalogue are
// marked retired rather than deleted, preserving existing grants. If a retired
// permission reappears, it is un-retired.
func (s *Store) syncPermissions(ctx context.Context) error {
	...
	return s.inTx(ctx, func(tx *sql.Tx) error {
		...
		// Retire pairs absent from the catalogue, un-retire those that returned.
		logFrom := logx.LogFrom{Primary: "auth", Secondary: "permissions"}
		now := timeNow().Format(timeFormat)
		for permission, id := range existing {
			if valid[permission] {
				if _, err := tx.ExecContext(ctx,
					`UPDATE permissions SET retired_at = NULL WHERE id = ? AND retired_at IS NOT NULL;`,
					id,
				); err != nil {
					return err //nolint:wrapcheck
				}
			...
			result, err := tx.ExecContext(ctx,
				`UPDATE permissions SET retired_at = ? WHERE id = ? AND retired_at IS NULL;`,
				now, id,
			)
			if err != nil {
				return err //nolint:wrapcheck
			}
			// Only report the pairs retired by this run.
			...
			if err := tx.QueryRowContext(ctx,
				`SELECT COUNT(*) FROM group_permissions WHERE permission_id = ?;`,
				id,
			).Scan(&grants); err != nil {
				return err //nolint:wrapcheck
			}
			logx.Warn(
				fmt.Sprintf(
					"retired permission %s:%s (no longer in the catalogue), %d group grant(s) kept but inactive",
...
var seededGroups = []struct {
	...
	},
	{
		...
		func(p rbac.Permission) bool { return p.Resource != rbac.ResourceMetric },
	},
	{
		GroupViewer, GroupViewer, false,
		"Read-only dashboard access.",
		func(p rbac.Permission) bool {
			return p.Action == rbac.ActionRead && p.Resource != rbac.ResourceMetric
		},
	},
}
