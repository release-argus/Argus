...
// argon2id parameters for new hashes.
const (
	argonTime    uint32 = 2
...
func decodeHash(encoded string) (
	...
	// Reject a too-short salt/key.
	if uint32(len(salt)) < saltLength/2 || uint32(len(key)) < keyLength/2 {
		return fail(fmt.Errorf("%w: salt/key too short", ErrMalformedHash))
	}
