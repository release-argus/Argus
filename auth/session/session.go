...
// DefaultMaxSessionsPerUser is the cap applied when [Config.MaxPerUser] is
// unset; starting a session past the cap evicts the least-recently active.
const DefaultMaxSessionsPerUser = 10

// DefaultLifetime is the session lifetime applied when [Config.Lifetime] is
// unset: 30 days.
const DefaultLifetime = "720h"

// DefaultIdleTimeout is the session idle timeout applied when
// [Config.IdleTimeout] is unset: 7 days.
const DefaultIdleTimeout = "168h"
...
type Manager struct {
	store  Store
	config Config
	// How often a session's last-seen time is flushed to the store.
...
func New(sessionStore Store, config Config) *Manager {
	// Flush at most every [touchInterval], but never
	// less often than half the idle timeout.
	touchEvery := touchInterval
	if half := config.IdleTimeout / 2; half > 0 && half < touchEvery {
		touchEvery = half
	}
...
) (token string, evicted []string, err error) {
...
func (m *Manager) evict(ctx context.Context, tokenHash string) {
	m.mu.Lock()
	delete(m.cache, tokenHash)
	m.gen++
	m.mu.Unlock()
...
// flushDue reports whether session's last-seen is due a flush to the store,
// advancing its in-memory marker to now when it is.
func (m *Manager) flushDue(session *store.Session, now time.Time) bool {
	if now.Sub(session.LastSeenAt) < m.touchEvery {
		return false
	}
...
// touchLastSeen flushes the session's last-seen time to the store.
func (m *Manager) touchLastSeen(ctx context.Context, tokenHash string, now time.Time) {
...
func (m *Manager) Validate(ctx context.Context, token string) (*store.Session, error) {
	...
	if cached {
		if m.expired(session, now) {
			m.mu.Unlock()
			m.evict(ctx, tokenHash)
			return nil, ErrInvalidSession
		}
		// Slide the idle window, cache, and touch DB.
		flush := m.flushDue(&session, now)
		if flush {
			m.cache[tokenHash] = session
		}
		m.mu.Unlock()
		if flush {
			m.touchLastSeen(ctx, tokenHash, now)
		}
		return &session, nil
	}
	gen := m.gen
