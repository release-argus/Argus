...
// Name is the local provider name.
const Name = "local"
...
// errMalformedHash aliases [password.ErrMalformedHash] (overridable for tests).
var errMalformedHash = password.ErrMalformedHash
...
func (p *Provider) Authenticate(
	...
	// Verify against a dummy hash for unknown users so response timing does not
	...
	if err != nil {
		if errors.Is(err, errMalformedHash) {
			logx.Error(
				fmt.Errorf(
					"stored password hash for %q is unusable: %w",
					username, err,
				),
				logx.LogFrom{Primary: "auth", Secondary: "local"},
				true,
			)
